class Calendar {
    ...
    static toDayDate = Calendar.getDateObj(new Date())
    static getDate (date) {
        date = Calendar.transFormDate(date)
        // 删除格数
        let dist
    ...
    // 头部过滤
    static getDateStr = (dataStr) => dataStr.slice(0, 7).replace("/", "-")
    
    // 设置上个月的时间
    static _getPrevMonth (monthFormat) {
    ...
    // 设置下个月的时间
    static _getNextMonth(monthFormat) {
        const [year, month] = monthFormat.split(/\-|\//);
